...
//        String studentName = AppDatabase.getDatabaseInstance(context).getStudentDao().getFullName(Assessment_Constants.currentStudentID);
    ...
    public void getStudentAvatar() {
        String studentAvatar = AppDatabase.getDatabaseInstance(context).getStudentDao().getStudentAvatar(Assessment_Constants.currentStudentID);
        if (studentAvatar != null)
            certificateView.setStudentAvatar(studentAvatar);
        else {
            String img = Assessment_Utility.getRandomAvatarNames(context);
            String[] imgArr = img.split("\\.");
            certificateView.setStudentAvatar(imgArr[0]);
        }
    }*/
